...
export default function ComputerUploadPanel({
  ...
  const describeError = (cause: unknown) => {
    ...
    const guidance = uploadFailureGuidance(responseStatus(cause));
  ...
  const refreshAfterStateError = async (cause: unknown) => {
    ...
    if (status === 404) {
      ...
      setMessage('The upload changed state on the database host. Its current state has been refreshed.');
    }
  ...
  const inspect = async () => {
    ...
    try {
      ...
      if (status === 429 || status === 503) {
        setMessage(`${describeError(cause)} You can still run a full import.`);
      } else {
        setMessage(describeError(cause));
      }
      await refreshAfterStateError(cause);
