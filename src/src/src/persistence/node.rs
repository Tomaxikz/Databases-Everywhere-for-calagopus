...
impl NodeRecord {
    ...
    pub fn into_api(self) -> NodeRecordApi {
        ...
        let mutation_block_reason = mutation_contract_block_reason(self.cached_system.as_ref());
        NodeRecordApi {
            uuid: self.uuid,
            name: self.name,
    ...
    pub async fn store_health(
        ...
        safe_query!((r#"
            ...
            SET cached_system = COALESCE($2, cached_system),
                ...
                last_seen = CASE WHEN $2::jsonb IS NOT NULL OR $3::jsonb IS NOT NULL THEN now() ELSE last_seen END,
                last_error = $4,
                updated = now()
            WHERE uuid = $1
            "#,))
    ...
    pub async fn mark_restart_required(
        &self,
        state: &State,
        restart_required: bool,
    ) -> Result<(), anyhow::Error> {
        safe_query!(
            ("UPDATE {NODES_TABLE} SET restart_required = $2, updated = now() WHERE uuid = $1")
        )
        .bind(self.uuid)
