...
const configurationSchema = z.object({
  ...
  backups: z.object({
    ...
    retentionKeepLatestPerInstance: z.number().int().min(1).max(100_000),
    ...
    browsing: z.object({
      enabled: z.boolean(),
      maxObjects: z.number().int().min(1).max(1_000),
      maxPreviewObjects: z.number().int().min(0).max(1_000),
      previewRowsPerObject: z.number().int().min(0).max(100),
      maxRowBytes: z.number().int().min(256).max(16_384),
      maxCatalogBytes: z.number().int().min(65_536).max(1_048_576),
    }),
  }),
  artifacts: z.object({
    streamExportsOnly: z.boolean(),
    maxArtifactsPerInstance: z.number().int().min(1).max(10_000),
    retentionKeepLatest: z.number().int().min(1).max(100_000),
    retentionMaxAgeDays: z.number().int().min(0).max(36_500),
    importUploadMaxBytes: z.number().int().min(1).max(MAX_IMPORT_BYTES),
    importUploadMaxTotalBytes: z.number().int().min(1).max(Number.MAX_SAFE_INTEGER),
    importUploadMaxPerInstance: z.number().int().min(1).max(64),
    importUploadMaxConcurrent: z.number().int().min(1).max(32),
    importUploadTtlHours: z.number().int().min(1).max(168),
    importUploadTimeoutSeconds: z.number().int().min(60).max(86_400),
    importUploadIdleTimeoutSeconds: z.number().int().min(5).max(300),
    scheduler: z.object({
  ...
  security: z.object({
    ...
    pidsLimits: z.object({
      postgres: z.number().int().min(0),
      mysql: z.number().int().min(0),
      mariadb: z.number().int().min(0),
      redis: z.number().int().min(0),
      valkey: z.number().int().min(0),
      mongodb: z.number().int().min(0),
      clickhouse: z.number().int().min(0),
      qdrant: z.number().int().min(0),
    }),
    remoteImport: z.object({
      enabled: z.boolean(),
      allowPlaintext: z.boolean(),
      allowedPrivateHosts: z.array(z.string().trim().min(1).max(253)),
      maxConcurrentJobs: z.number().int().min(1).max(64),
      connectTimeoutSeconds: z.number().int().min(1).max(300),
      operationTimeoutSeconds: z.number().int().min(1).max(86_400),
      maxStagedBytes: z.number().int().min(1).max(MAX_IMPORT_BYTES),
    }),
  }),
  disk: z.object({
    ...
    softScanner: z.object({
      ...
      useInotify: z.boolean(),
      fullScanIntervalSeconds: z.number().int().min(1).max(3_600),
      inotifyDebounceMilliseconds: z.number().int().min(1).max(60_000),
      maxDirtyPathsPerInstance: z.number().int().min(1).max(65_536),
...
export function hasUnsupportedSelfUpgrade(configuration: Record<string, unknown> | null | undefined): boolean {
  const security = configuration?.security;
  return (
    Boolean(security) &&
    typeof security === 'object' &&
    !Array.isArray(security) &&
    (security as Record<string, unknown>).self_upgrade_enabled === true
  );
}
...
export function createNodeFormSchema(messages: {
  ...
  schedulerLimitExceedsQueue: string;
  backupPreviewExceedsObjects: string;
  remotePrivateHostInvalid: string;
  remoteOperationTimeoutTooShort: string;
  uploadTotalSmallerThanFile: string;
  uploadIdleTimeoutTooLong: string;
}) {
  return baseNodeFormSchema.superRefine((value, context) => {
    ...
    if (value.config.backups.browsing.maxPreviewObjects > value.config.backups.browsing.maxObjects) {
      context.addIssue({
        code: 'custom',
        path: ['config', 'backups', 'browsing', 'maxPreviewObjects'],
        message: messages.backupPreviewExceedsObjects,
      });
    }
    ...
    for (const [field, fieldValue] of [
      ...
      if (fieldValue > scheduler.maxQueuedJobs) {
        context.addIssue({
          ...
          message: messages.schedulerLimitExceedsQueue,
        });
      }
    }
    const remoteImport = value.config.security.remoteImport;
    if (remoteImport.allowedPrivateHosts.some((host) => !isRemoteImportHost(host))) {
      context.addIssue({
        code: 'custom',
        path: ['config', 'security', 'remoteImport', 'allowedPrivateHosts'],
        message: messages.remotePrivateHostInvalid,
      });
    }
    if (remoteImport.operationTimeoutSeconds < remoteImport.connectTimeoutSeconds) {
      context.addIssue({
        code: 'custom',
        path: ['config', 'security', 'remoteImport', 'operationTimeoutSeconds'],
        message: messages.remoteOperationTimeoutTooShort,
      });
    }
    if (value.config.artifacts.importUploadMaxTotalBytes < value.config.artifacts.importUploadMaxBytes) {
      context.addIssue({
        code: 'custom',
        path: ['config', 'artifacts', 'importUploadMaxTotalBytes'],
        message: messages.uploadTotalSmallerThanFile,
      });
    }
    if (value.config.artifacts.importUploadIdleTimeoutSeconds > value.config.artifacts.importUploadTimeoutSeconds) {
      context.addIssue({
        code: 'custom',
        path: ['config', 'artifacts', 'importUploadIdleTimeoutSeconds'],
        message: messages.uploadIdleTimeoutTooLong,
      });
    }
  });
}
...
function isRemoteImportHost(value: string) {
  const host = value.trim();
  if (!host) return false;
  const ipv4Parts = host.split('.');
  if (ipv4Parts.length === 4 && ipv4Parts.every((part) => /^(?:0|[1-9]\d{0,2})$/.test(part) && Number(part) <= 255)) {
    return true;
  }
  const ipv6 = host.startsWith('[') && host.endsWith(']') ? host.slice(1, -1) : host;
  if (ipv6.includes(':')) {
    try {
      return new URL(`http://[${ipv6}]`).hostname.length > 2;
    } catch {
      return false;
    }
  }
  const normalized = host.endsWith('.') ? host.slice(0, -1) : host;
  if (!normalized || normalized.length > 253 || [...normalized].some((character) => character.charCodeAt(0) > 0x7f)) {
    return false;
  }
  ...
  return !labels.every((label) => /^\d+$/.test(label) || /^0x[\da-f]+$/i.test(label));
}
...
const nullableOverrideValue = (root: Record<string, unknown>, path: string[], fallback: number) => {
  let current: unknown = root;
  for (const part of path) {
    if (!current || typeof current !== 'object' || Array.isArray(current)) return fallback;
    current = (current as Record<string, unknown>)[part];
...
export function initialNodeFormValues(node?: NodeRecord): NodeFormValues {
  ...
  const configuredLogsPath = stringValue(configuration, ['paths', 'logs'], '/var/lib/dbev/logs');
  const protocolValues = Object.fromEntries(
    protocols.map(([protocol, , defaultPort, defaultImage]) => {
      ...
      const base = {
        ...
        allowedImages: stringsValue(
          configuration,
          ['images', 'allowed', protocol],
          protocol === 'mongodb' ? [defaultImage, 'mongo:7.0.37'] : [defaultImage],
        ),
      };
      return [
        protocol,
  ...
  return {
    ...
    config: {
      ...
      backups: {
        enabled: booleanValue(configuration, ['backups', 'enabled'], false),
      ...
      artifacts: {
        ...
        importUploadMaxBytes: numberValue(configuration, ['artifacts', 'import_upload_max_bytes'], 8_589_934_592),
        importUploadMaxTotalBytes: numberValue(
          configuration,
          ['artifacts', 'import_upload_max_total_bytes'],
          34_359_738_368,
        ),
        importUploadMaxPerInstance: numberValue(configuration, ['artifacts', 'import_upload_max_per_instance'], 4),
        importUploadMaxConcurrent: numberValue(configuration, ['artifacts', 'import_upload_max_concurrent'], 2),
        importUploadTtlHours: numberValue(configuration, ['artifacts', 'import_upload_ttl_hours'], 24),
        importUploadTimeoutSeconds: numberValue(configuration, ['artifacts', 'import_upload_timeout_seconds'], 3_600),
        importUploadIdleTimeoutSeconds: numberValue(
          configuration,
      ...
      security: {
        ...
        pidsLimits: {
          postgres: nullableOverrideValue(configuration, ['security', 'pids_limits', 'postgres'], 0),
          mysql: nullableOverrideValue(configuration, ['security', 'pids_limits', 'mysql'], 0),
          mariadb: nullableOverrideValue(configuration, ['security', 'pids_limits', 'mariadb'], 0),
          redis: nullableOverrideValue(configuration, ['security', 'pids_limits', 'redis'], 0),
          valkey: nullableOverrideValue(configuration, ['security', 'pids_limits', 'valkey'], 0),
          mongodb: nullableOverrideValue(configuration, ['security', 'pids_limits', 'mongodb'], 0),
          clickhouse: nullableOverrideValue(configuration, ['security', 'pids_limits', 'clickhouse'], 4_096),
          qdrant: nullableOverrideValue(configuration, ['security', 'pids_limits', 'qdrant'], 0),
        },
        remoteImport: {
          enabled: booleanValue(configuration, ['security', 'remote_import', 'enabled'], true),
          allowPlaintext: booleanValue(configuration, ['security', 'remote_import', 'allow_plaintext'], false),
          allowedPrivateHosts: stringsValue(configuration, ['security', 'remote_import', 'allowed_private_hosts'], []),
          maxConcurrentJobs: numberValue(configuration, ['security', 'remote_import', 'max_concurrent_jobs'], 4),
          ...
          maxStagedBytes: numberValue(configuration, ['security', 'remote_import', 'max_staged_bytes'], 8_589_934_592),
        },
      },
      disk: {
        mode: (() => {
        ...
        softScanner: {
          ...
          useInotify: booleanValue(configuration, ['disk', 'soft_scanner', 'use_inotify'], true),
          fullScanIntervalSeconds: numberValue(
            configuration,
            ['disk', 'soft_scanner', 'full_scan_interval_seconds'],
            90,
          ),
          inotifyDebounceMilliseconds: numberValue(
            configuration,
            ['disk', 'soft_scanner', 'inotify_debounce_milliseconds'],
            500,
          ),
          maxDirtyPathsPerInstance: numberValue(
            configuration,
      ...
      paths: {
        ...
        logs: configuredLogsPath === '/var/log/dbev' ? '/var/lib/dbev/logs' : configuredLogsPath,
...
export function toNodeInput(
  ...
  const configuration = mergeConfiguration(configurationBase, {
    ...
    security: {
      ...
      pids_limits: Object.fromEntries(
        protocols.map(([protocol]) => [protocol, config.security.pidsLimits[protocol] || null]),
      ),
      remote_import: {
        enabled: config.security.remoteImport.enabled,
        allow_plaintext: config.security.remoteImport.allowPlaintext,
        allowed_private_hosts: config.security.remoteImport.allowedPrivateHosts,
        max_concurrent_jobs: config.security.remoteImport.maxConcurrentJobs,
        connect_timeout_seconds: config.security.remoteImport.connectTimeoutSeconds,
        operation_timeout_seconds: config.security.remoteImport.operationTimeoutSeconds,
        max_staged_bytes: config.security.remoteImport.maxStagedBytes,
      },
    },
    artifacts: {
      ...
      import_upload_max_bytes: config.artifacts.importUploadMaxBytes,
      import_upload_max_total_bytes: config.artifacts.importUploadMaxTotalBytes,
      import_upload_max_per_instance: config.artifacts.importUploadMaxPerInstance,
      import_upload_max_concurrent: config.artifacts.importUploadMaxConcurrent,
      import_upload_ttl_hours: config.artifacts.importUploadTtlHours,
      import_upload_timeout_seconds: config.artifacts.importUploadTimeoutSeconds,
      import_upload_idle_timeout_seconds: config.artifacts.importUploadIdleTimeoutSeconds,
    ...
    disk: {
      ...
      soft_scanner: {
        ...
        use_inotify: config.disk.softScanner.useInotify,
        full_scan_interval_seconds: config.disk.softScanner.fullScanIntervalSeconds,
        inotify_debounce_milliseconds: config.disk.softScanner.inotifyDebounceMilliseconds,
        max_dirty_paths_per_instance: config.disk.softScanner.maxDirtyPathsPerInstance,
