...
import type { DatabaseProtocol, NodeCredentials, NodeRecord } from '../api/types.ts';
...
import { nodeRuntimeState } from './nodeRuntimeState.ts';
...
export default function NodeWorkspace({ node, onDeleted, onCredentials }: Props) {
  ...
  const runtimeState = nodeRuntimeState(node);
  ...
  const applyPatch = async () => {
    ...
    setAction('patch');
    try {
      const result = await patchRemoteConfiguration(node.uuid, patch);
      setLiveResult(result);
      setLiveResultKind('patch');
      addToast(t('server.operationQueued', {}), 'success');
      await queryClient.invalidateQueries({ queryKey: dbevQueryKeys.adminNodes() });
    } catch (error) {
      const message = httpErrorToHuman(error);
      setPatchFailure(message);
  ...
  const doDelete = async () => {
    if (runtimeState.recordMutationBlocked || confirmName !== node.name) return;
    setAction('delete');
    try {
  ...
  return (
    <Stack>
      {runtimeState.recordMutationBlocked && (
        <Alert color='yellow' title={t('admin.readOnlyCompatibilityMode', {})}>
          {runtimeState.mutationBlockReason || t('admin.readOnlyCompatibilityDescription', {})}
        </Alert>
      )}
      {runtimeState.mutationBlocked && !runtimeState.recordMutationBlocked && (
        <Alert color='yellow' title={t('admin.unverifiedContractMode', {})}>
          {t('admin.unverifiedContractDescription', {})}
        </Alert>
      )}
      {runtimeState.restartRequired && (
        <Alert color='yellow' title={t('admin.restartRequired', {})}>
          <Stack gap='xs'>
            <Text size='sm'>{t('admin.restartRequiredDescription', {})}</Text>
            <Group>
              {canUseCredentials && (
                <Button
                  size='compact-xs'
                ...
                >
                  {t('admin.viewGeneratedConfig', {})}
                </Button>
              )}
              {canRunOperations && (
                <Button
                  size='compact-xs'
                  loading={action === 'restart-confirm'}
                  onClick={async () => {
                    const result = await run('restart-confirm', () =>
                      nodeOperation(node.uuid, 'restart-confirm', 'post'),
                    );
                    if (result) await queryClient.invalidateQueries({ queryKey: dbevQueryKeys.adminNodes() });
                  }}
                >
                  {t('admin.checkAfterRestart', {})}
                </Button>
              )}
            </Group>
          </Stack>
      ...
      <Group justify='space-between'>
        <Group>
          <StatusBadge status={node.enabled ? (runtimeState.operationalError ? 'failed' : 'healthy') : 'disabled'} />
        ...
        <Group>
          <AdminCan action='databases-everywhere-nodes.update'>
            <Button
              variant='default'
              disabled={runtimeState.recordMutationBlocked}
              onClick={() => navigate(`/admin/databases-everywhere/${node.uuid}/edit`)}
            >
      ...
      <Tabs value={activeTab} onChange={(value) => value && setActiveTab(value)} keepMounted={false}>
        ...
        {(canUseCredentials || canUpdate) && (
          <Tabs.Panel value='configuration' pt='md'>
            <Stack>
              {patchFailure && (
                <Alert color='red' title={t('admin.livePatchFailed', {})}>
                  <Stack gap='xs'>
                    <Text size='sm'>{patchFailure}</Text>
                    <Text size='xs'>{t('admin.livePatchFailedDescription', {})}</Text>
                    <Group>
                      <Button size='compact-xs' loading={action === 'patch'} onClick={() => void applyPatch()}>
                        {t('admin.retryPatch', {})}
                      </Button>
                      {canUseCredentials && (
                        <Button size='compact-xs' variant='default' onClick={() => void configuration.refetch()}>
                          {t('admin.viewGeneratedConfig', {})}
                        </Button>
                      )}
                    </Group>
              ...
              {canUpdate && (
                <Card>
                  ...
                  <Group justify='flex-end' mt='md'>
                    <Button
                      loading={action === 'allocation-guards'}
                      disabled={runtimeState.mutationBlocked}
                      onClick={applyAllocationGuards}
                    >
              ...
              {canUpdate && <ArtifactPolicyPanel node={node} disabled={runtimeState.mutationBlocked} />}
              {canUpdate && (
                <SchedulerPanel
                  node={node}
      ...
      <Modal opened={deleteOpen} onClose={() => setDeleteOpen(false)} title={t('admin.deleteNode', {})}>
        ...
        <ModalFooter>
          <Button
            ...
            disabled={runtimeState.recordMutationBlocked || confirmName !== node.name}
            onClick={doDelete}
          >
            {t('common.delete', {})}
          </Button>
