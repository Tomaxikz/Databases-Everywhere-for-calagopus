...
export default function ArtifactPolicyPanel({ node, disabled = false }: { node: NodeRecord; disabled?: boolean }) {
  ...
  const save = async () => {
    if (saving || validationError || disabled) return;
    setSaving(true);
    try {
      await patchRemoteConfiguration(node.uuid, artifactPolicyPatch(configuration));
