...
export default function DatabaseViewPage() {
  ...
  return (
    <>
                  ...
                  <Menu.Item
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (failure.requiresReconciliation) await query.refetch();
                        addToast(
                          failure.retryable
      ...
      >
        {mutationBlocked && (
          <Alert color='yellow' title='Read-only compatibility mode' mb='md'>
            {database.mutation_block_reason ||
              'This database remains visible for diagnostics, but state-changing actions are disabled until its DBEV node matches API 0.12.0.'}
          </Alert>
        )}
        <DatabaseWorkspace
          server={server.uuid}
