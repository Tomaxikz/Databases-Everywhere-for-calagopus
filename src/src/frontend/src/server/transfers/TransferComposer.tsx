...
export default function TransferComposer({
  ...
  const remoteImportState = remoteImportSourceState(database.remote_import_enabled);
  ...
  useEffect(() => {
    if (!database.remote_import_enabled && sourceType === 'remote') setSourceType('saved');
  }, [database.remote_import_enabled, sourceType]);
  useEffect(() => {
    if (!database.remote_import_allow_plaintext && !remote.tls) {
      setRemote((current) => ({ ...current, tls: true }));
    }
  }, [database.remote_import_allow_plaintext, remote.tls]);
  ...
  return (
    <Stack gap='md'>
      {!database.mutations_allowed && (
        <Alert color='yellow' title='Read-only compatibility mode'>
          {database.mutation_block_reason ||
            'This database host can be inspected, but imports and exports are disabled until its API contract matches this extension.'}
        </Alert>
      )}
            ...
            <Group justify='flex-end' mt='lg'>
              <ServerCan action='databases-everywhere.export'>
                <Button
                  ...
                  disabled={!database.mutations_allowed || (selectiveExport && !exportInclude.length)}
                >
                  Queue export
                </Button>
              </ServerCan>
        ...
        >
          <SegmentedControl
            ...
            data={[
              ...
              { value: 'remote', label: 'Another database', disabled: remoteImportState.disabled },
            ]}
          />
                ...
                {sourceType === 'remote' ? (
                  <>
                    ...
                    <RemoteSourceFields
                      ...
                      value={remote}
                      onChange={setRemote}
                      allowPlaintext={database.remote_import_allow_plaintext}
                    />
