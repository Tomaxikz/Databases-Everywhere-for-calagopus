...
export function uploadFailureGuidance(status: number | null): string | null {
  switch (status) {
    ...
    case 408:
      return 'The upload stopped sending data and exceeded the node’s idle or total upload timeout.';
    case 409:
      return null;
    case 413:
      return 'This dump exceeds the maximum upload size configured for this DBEV node.';
    case 415:
      return 'DBEV expected a JSON request or a raw application/octet-stream upload.';
    case 429:
      return 'The DB Agent upload or inspection queue is busy. Try again shortly.';
    case 503:
      return 'DBEV could not inspect this upload right now. Full import may still be available; refresh the upload state before retrying inspection.';
    case 500:
