...
export default function UpdatesTab({ server, database, onChanged }: Props) {
  ...
  const run = async (name: 'image' | 'reconcile', operation: () => Promise<unknown>, success: string) => {
    ...
    try {
      ...
      if (nextFailure.requiresReconciliation) {
  ...
  return (
    <Stack mt='md'>
      ...
      <MutationFailureAlert failure={failure} checking={checkingState} onCheck={() => void checkStateBeforeRetry()} />
          ...
          <Stack>
            ...
            <Group justify='flex-end'>
              <Button
                ...
                disabled={mutationBlocked || action !== null || progressRunning || !policy.reconcile}
                onClick={() =>
